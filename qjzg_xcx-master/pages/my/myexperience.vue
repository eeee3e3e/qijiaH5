	...
	<view class="content">
		<view class="search">
			<view class="main">
				...
				<input @input="goSearch" confirm-type="搜索" @confirm="confirmTap" v-model="search" placeholder="请输入查找内容" placeholder-style="color:#aeaeae" />
			</view>
		</view>
		<view class="mainbody" v-if="hotnotelist_one != ''">
			<view v-for="(item, index) in hotnotelist_one" :key="index">
				<view class="node" @tap="goto(`/pages/experience/paste?ID=` + 1 + '&key=' + hotnotelist_one[index].ID)">
					<view class="title">{{ hotnotelist_one[index].Title }}</view>
					<view class="content">
						<view class="text">{{ hotnotelist_one[index].Experience }}</view>
						<view class="img"><image :src="'https://www.qijiazhiguo.top' + hotnotelist_one[index].Urls[0]" mode=""></image></view>
					</view>
					<view class="extraaa">
						<image :src="hotnotelist_one[index].HeadImage" mode=""></image>
...
<script>
	export default {
		...
		onLoad() {
			uni.request({
				url: 'https://www.qijiazhiguo.top/api/UserMsg/GetMyExperience',
